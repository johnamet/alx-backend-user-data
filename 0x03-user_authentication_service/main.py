...
def register_user(email: str, password: str) -> None:
    ...
    assert payload["message"] == "user created", f"Expected message 'user created', got {payload['message']}"
...
def log_in(email: str, password: str) -> str:
    ...
    payload = response.json()
    assert payload["email"] == email, f"Expected email {email}, got {payload['email']}"
    assert payload["message"] == "logged in", f"Expected message 'logged in', got {payload['message']}"
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    ...
    payload = response.json()
    assert payload["email"] == email, f"Expected email {email}, got {payload['email']}"
    assert payload["message"] == "Password updated", f"Expected message 'Password updated', got {payload['message']}"
